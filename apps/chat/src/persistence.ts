...
// Resolved LAZILY (per call), not at import time: if module evaluation ever races
// ahead of the Tauri shell injecting `window.__ARETE_API_BASE__`, an eager constant
// would freeze the wrong (relative) base and every API call would 404. This mirrors
// agui-client.ts's lazy strategy.
const apiOrigin = (): string =>
  (typeof window !== 'undefined' && (window as { __ARETE_API_BASE__?: string }).__ARETE_API_BASE__) ||
  '';
const BASE = (): string => `${apiOrigin()}/api`;
