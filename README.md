...
- **Cursor Next/Prev DataTable mode** — pagination today defaults to client-side (fetch the rows once, paginate in-browser), which is reliable for bounded datasets but requires fetching everything up front. For genuinely large, cursor-paginated tools (opaque sequential tokens, no jump-to-page), add a first-class server-paging mode: a prev/next-only paginator (no page numbers) with `hasNext`/`hasPrev` flags and a directional `pageAction` that carries the stored opaque cursor — so the agent never has to map a row offset (`ctx.first`) to a cursor. Until then, large lists use a capped client-side fetch ("first N").
